...
if (isset($_POST['action']) && $_POST['action'] == 'add_payment') {
    ...
    if ($stmt->get_result()->num_rows > 0) {
        ...
        $status = $_POST['default_status'] ?? 'pending';
        $stmt = $conn->prepare("INSERT INTO payment_options (name, default_status) VALUES (?, ?)");
        $stmt->bind_param("ss", $_POST['name'], $status);
        $stmt->execute();
...
if (isset($_POST['action']) && $_POST['action'] == 'edit_payment') {
    $stmt = $conn->prepare("SELECT id FROM payment_options WHERE name = ? AND id != ?");
    $stmt->bind_param("si", $_POST['name'], $_POST['id']);
    $stmt->execute();
    if ($stmt->get_result()->num_rows > 0) {
        $error = 'Another payment option with this name already exists';
    } else {
        $status = $_POST['default_status'] ?? 'pending';
        $stmt = $conn->prepare("UPDATE payment_options SET name = ?, default_status = ? WHERE id = ?");
        $stmt->bind_param("ssi", $_POST['name'], $status, $_POST['id']);
        $stmt->execute();
        header("Location: manage_payments.php");
        exit;
    }
...
<body>
                            ...
                            <tbody>
                                <?php foreach ($payments as $pay): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge <?php echo $pay['default_status'] == 'paid' ? 'bg-success' : 'bg-warning text-dark'; ?>">
                                                <?php echo ucfirst($pay['default_status']); ?>
                                            </span>
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-warning editPaymentBtn me-1" data-id="<?php echo $pay['id']; ?>" data-name="<?php echo htmlspecialchars($pay['name']); ?>" data-status="<?php echo htmlspecialchars($pay['default_status']); ?>">
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header" style="background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); color: white;">
                                <h5 class="modal-title"><i class="fas fa-edit"></i> Edit Payment Option</h5>
                                <button type="button" class="close" id="editPaymentCloseX" style="color: white; opacity: 0.8;"><span>&times;</span></button>
                            </div>
                            <form method="post">
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label">Payment Option Name</label>
                                        <input type="text" name="name" id="editPaymentName" class="form-control" required placeholder="Enter payment option name">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Default Status</label>
                                        <select name="default_status" id="editPaymentStatus" class="form-control" required>
                                            <option value="pending">Pending</option>
                                            <option value="paid">Paid</option>
                                        </select>
                                    </div>
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" id="editPaymentCancel">Cancel</button>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Cancel / X close handlers
            $('#editPaymentCancel, #editPaymentCloseX').click(function() {
