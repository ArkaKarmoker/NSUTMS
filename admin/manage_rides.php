        ...
        function loadRides() {
            ajaxRequest('../ajax/ride_actions.php', {action: 'get_rides'}, function(response) {
                ...
                response.forEach(function(ride) {
                    ...
                    if(ride.status === 'completed' || ride.status === 'ended') statusBadge = '<span class="badge bg-success" style="color: white !important;">' + (ride.status.charAt(0).toUpperCase() + ride.status.slice(1)) + '</span>';
                    else if(ride.status === 'started') statusBadge = '<span class="badge bg-primary" style="color: white !important;">Started</span>';
                    else if(ride.status === 'cancelled') statusBadge = '<span class="badge bg-danger" style="color: white !important;">Cancelled</span>';
                    else statusBadge = '<span class="badge bg-secondary" style="color: white !important;">' + ride.status + '</span>';
