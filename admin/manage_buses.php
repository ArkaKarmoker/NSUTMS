        ...
        function loadBuses() {
            ajaxRequest('../ajax/bus_actions.php', {action: 'get_buses'}, function(response) {
                ...
                response.forEach(function(bus) {
                    html += '<tr><td>' + bus.id + '</td><td>' + bus.reg_number + '</td><td>' + bus.seats + '</td><td>' + (Number(bus.is_female_only) ? '<span class="badge" style="background-color: #e83e8c !important; color: white !important;">Yes</span>' : '<span class="badge" style="background-color: #0dcaf0 !important; color: white !important;">No</span>') + '</td><td><button class="btn btn-sm btn-info editBus" data-id="' + bus.id + '" data-reg_number="' + bus.reg_number + '" data-seats="' + bus.seats + '" data-is_female_only="' + bus.is_female_only + '"><i class="fas fa-edit"></i> Edit</button> <button class="btn btn-sm btn-danger deleteBus" data-id="' + bus.id + '"><i class="fas fa-trash"></i> Delete</button></td></tr>';
                });
                html += '</tbody></table>';
