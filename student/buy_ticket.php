...
<body>
    <div class="admin-container">
        ...
        <div class="admin-main">
            ...
            <div class="dashboard-content">
                ...
                <div class="content-card">
                    ...
                    <form id="buyTicketForm" class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <small id="availableSeatsDisplay" class="text-muted d-block mt-1"></small>
                        </div>
                ...
                <div class="footer">
                    <p>NSUTMS &copy; <?php echo date('Y'); ?> | Student Portal</p>
                </div>
    ...
    <script>
        function updateTimes() {
            ...
            var tripDate = $('input[name="trip_date"]').val();
            ...
            if (destId) {
                ajaxRequest('../ajax/destination_actions.php', {action: 'get_times', destination_id: destId}, function(response) {
                    ...
                    if (Array.isArray(response)) {
                        ...
                        var currentTimeStr = String(now.getHours()).padStart(2, '0') + ':' + String(now.getMinutes()).padStart(2, '0') + ':' + String(now.getSeconds()).padStart(2, '0');
                        
                        response.forEach(function(timeObj) {
                            if (tripDate && tripDate === todayStr) {
                                if (timeObj.time > currentTimeStr) {
                                    $('#timeSelect').append('<option value="' + timeObj.id + '">' + timeObj.time + '</option>');
                                }
                            } else {
                                $('#timeSelect').append('<option value="' + timeObj.id + '">' + timeObj.time + '</option>');
                            }
        ...
        $('#destSelect, input[name="trip_date"]').change(function() {
            updateTimes();
        });
        ...
        function checkAvailableSeats() {
            ...
            var femaleOnly = $('select[name="female_only"]').length ? $('select[name="female_only"]').val() : 0;
            
            if (tripDate && destId && timeId) {
                ajaxRequest('../ajax/ticket_actions.php', {
                    action: 'check_seats',
                    trip_date: tripDate,
                    ...
                    if (response.success) {
                        $('#availableSeatsDisplay').html('Available seats: <strong>' + response.available_seats + '</strong>');
                        $('input[name="seats"]').attr('max', response.available_seats);
                        
                        if (response.available_seats <= 0) {
                            $('#availableSeatsDisplay').removeClass('text-muted').addClass('text-danger');
                            $('button[type="submit"]').prop('disabled', true);
                        } else {
                            $('#availableSeatsDisplay').removeClass('text-danger').addClass('text-muted');
                            $('button[type="submit"]').prop('disabled', false);
                        }
                    } else {
                        $('#availableSeatsDisplay').html('<span class="text-danger">' + (response.message || 'No bus available') + '</span>');
                        $('button[type="submit"]').prop('disabled', true);
                        $('input[name="seats"]').attr('max', 0);
                    }
                });
            } else {
                $('#availableSeatsDisplay').empty();
                $('button[type="submit"]').prop('disabled', false);
                $('input[name="seats"]').removeAttr('max');
            }
        }

        $('#timeSelect, select[name="female_only"]').change(function() {
            checkAvailableSeats();
        });
                ...
                if (response.success) {
                    if (response.payment_method == 'online') {
                        ajaxRequest('../ajax/payment_actions.php', {action: 'complete_payment', ticket_id: response.ticket_id}, function(payResp) {
                            ...
                            alert(payResp.message || 'Payment processing');
                        });
                    } else {
